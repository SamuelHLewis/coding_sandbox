...
This is a guide for setting up the OpenCode agentic coding harness to run inside a sandbox.
...
Finally, create an image for the container that contains the coding agent harness and its dependencies:
...
Once the podman VM is running, use the custom command defined in the previous section to launch the coding agent harness within a container. You should specify the path to your working dir as the first argument:
...
After entering the authorisation details that it requests, these will be saved within the files that the container mounts, so you shouldn't need to authorise when you start the agent harness again.

## Other Coding Harnesses

These instructions have been tested with OpenCode, a vendor-agnostic agentic coding harness which allows you to connect to your Anthropic, OpenAI, Google and other accounts. However, if you want to run other harnesses in the same containerised way, you can use the files in this repo and adjust:
